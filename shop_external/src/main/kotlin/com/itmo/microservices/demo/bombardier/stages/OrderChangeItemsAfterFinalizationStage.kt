        ...
        repeat(Random.nextInt(1, 5)) {
            ...
            ConditionAwaiter.awaitAtMost(3, TimeUnit.SECONDS)
                .condition {
                    ...
                    val firstCond = theOrder.itemsMap.any {
                        it.key.id == itemToAdd.id && it.value == amount
                    }
                    val secondCond = theOrder.status == OrderStatus.OrderCollecting
                    println("Order is upgraded: $firstCond")
                    println("Order is collecting: $secondCond")
